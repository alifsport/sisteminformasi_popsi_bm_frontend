    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Cari nama, ID anggota, atau no. telepon..."
              value={search}
              ...
              {isLoading ? (
                ...
                anggotaList.map((a) => (
                  <tr key={a.id} className="transition hover:bg-gray-50">
                    {/* ID Anggota */}
                    <td className="whitespace-nowrap px-4 py-3 font-mono text-xs font-medium text-gray-700">
                      {a.id_anggota}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_COLORS[a.status_keanggotaan] ?? 'bg-gray-100 text-gray-700'}`}
                      >
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => navigate(`/anggota/${a.id}`)}
                          className="rounded-md px-2.5 py-1 text-xs font-medium text-blue-600 transition hover:bg-blue-50"
                          title="Lihat detail"
                        >
                          Lihat
                        </button>
        ...
        {pagination && (
          <div className="flex flex-col items-center justify-between gap-3 border-t px-4 py-3 sm:flex-row">
            ...
            <div className="flex items-center gap-3 text-sm text-gray-500">
              <span>
                Total: <span className="font-medium text-gray-700">{pagination.total}</span> anggota
              </span>
              <select
                value={limit}
            ...
            {totalPages > 1 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="rounded-md border px-3 py-1 text-sm font-medium transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
                >
                  Prev
                </button>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => !deleting && setDeleteId(null)}>
          ...
          >
            <h3 className="text-lg font-bold text-gray-900">Hapus Anggota</h3>
